...
;definire structuri
(define-struct birdy (x y v-y)) ; pasare
(define-struct spipe (x gap-y)) ;pipe
(define-struct abilities (visible active)) ; contine 2 liste: abilitati vizibile & active
(define-struct variables (gravity momentum scroll-speed))
(define-struct cstate (birdy variables listpipes myscore))
...
;(define initial-state (bird bird-x bird-initial-y 0))
;(define initial-state (state (bird bird-x bird-initial-y 0)
 ;                              (pipe 0 0) (variables 0 0 0)))
(define (get-initial-state)
  (define bird1 (birdy bird-x bird-initial-y 0))
  (define pipe1 (spipe scene-width (+ added-number (random random-threshold))))
  (define var (variables initial-gravity initial-momentum initial-scroll-speed))
  (define initial-state (cstate bird1 var (list pipe1) 0))
  initial-state)
...
(define (next-state-bird-onspace bird momentum)
  (match-let ([(birdy _ _ v-y) bird])
    (struct-copy birdy bird [v-y (- momentum)])))
...
(define (change current-state pressed-key)
  (match-let ([(cstate birdy variables _ _) current-state])
    (cond [(key=? pressed-key " ") (struct-copy cstate current-state [birdy (next-state-bird-onspace birdy (variables-momentum variables))])]
          [else current-state])))
...
; scroll-speed-ul dat.   ; pipes = lista de pipe-uri
(define (move-pipes pipes scroll-speed)
  (map (lambda (p) (struct-copy spipe p [x (- (spipe-x p) scroll-speed)])) pipes))
;TODO 12
...
; șters.
; Funcția va primi drept parametru mulțimea pipe-urilor din stare. ; pipes = multimea de pipe-uri
;
; Hint: cunoaștem lățimea unui pipe, pipe-width
(define (clean-pipes pipes)
  (filter (lambda (p) (> (+ (spipe-x p) pipe-width) 0)) pipes)) 
...
(define (add-more-pipes pipes)
     (define new-pipe (spipe (+ pipe-width pipe-gap (spipe-x (last pipes))) (+ added-number (random random-threshold))))
...
(define (check-ground-collision bird)
 (if (>= (+ (birdy-y bird) bird-height) ground-y)
     #t
     #f))
...
(define (invalid-state? state)
  (match-let ([(cstate birdy variables listpipes myscore) state])
    (or (check-ground-collision birdy) (check-pipe-collisions birdy listpipes)))) 
...
(define (check-pipe-collisions bird pipes)
  (let iter ((pipes pipes))
    (if (null? pipes)
        #f
        (if (or (check-collision-rectangles (make-posn (birdy-x bird) (birdy-y bird)) (make-posn (+ (birdy-x bird) bird-width) (+ (birdy-y bird) bird-height))
                                            (make-posn (spipe-x (car pipes)) 0) (make-posn (+ (spipe-x (car pipes)) pipe-width) (spipe-gap-y (car pipes))))
                (check-collision-rectangles (make-posn (birdy-x bird) (birdy-y bird)) (make-posn (+ (birdy-x bird) bird-width) (+ (birdy-y bird) bird-height))
                                            (make-posn (spipe-x (car pipes)) (+ (spipe-gap-y (car pipes)) pipe-self-gap)) (make-posn (+ (spipe-x (car pipes)) pipe-width) ground-y)))
            #t
            (iter (cdr pipes))))))
...
(define (next-state state)
  (match-let ([(cstate birdy variables listpipes myscore) state])
    (struct-copy cstate state [birdy (next-state-bird birdy (variables-gravity variables))]
    [listpipes (next-state-pipes listpipes (variables-scroll-speed variables))]
    (myscore (+ myscore 0.1)))))
...
(define (draw-frame state)
  (match-let ([(cstate birdy variables listpipes myscore) state])
    (place-image bird-image (+ (birdy-x birdy) (quotient bird-width 2)) (+ (birdy-y birdy) (quotient bird-height 2))
                 (place-image ground-image (quotient scene-width 2) (+ ground-y (quotient ground-height 2))
                              (place-image (score-to-image myscore) text-x text-y (place-pipes listpipes initial-scene))))))
...
(define (place-pipes pipes scene)
  (if (null? pipes)
      scene
  (let ((first (car pipes)))
        (place-pipes (cdr pipes) (place-image (rectangle (add1 pipe-width) pipe-self-gap "solid" "white") (+ (spipe-x first) (quotient pipe-width 2)) (+ (spipe-gap-y first) (quotient pipe-self-gap 2))
                     (place-image (rectangle pipe-width  (* pipe-height 2) "solid" "green") (+ (spipe-x first) (quotient pipe-width 2)) (+ (spipe-gap-y first) (quotient pipe-self-gap 2)) scene))))))
...
(define get-variables variables)
(define get-variables-gravity variables-gravity)
(define get-variables-momentum 'variables-momentum)
(define get-variables-scroll-speed variables-scroll-speed)
